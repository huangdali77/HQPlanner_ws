...
bool StBoundary::IsPointNear(const hqplanner::math::LineSegment2d& seg,
...
bool StBoundary::IsPointInBoundary(const STPoint& st_point) const {
  ...
  const double check_upper = hqplanner::math::CrossProd(
      st_point, upper_points_[left], upper_points_[right]);
  const double check_lower = hqplanner::math::CrossProd(
