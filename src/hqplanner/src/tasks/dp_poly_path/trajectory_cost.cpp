...
TrajectoryCost::TrajectoryCost(
  ...
  const float total_time =
      std::min(heuristic_speed_data_.TotalTime(),
  ...
  for (const auto *ptr_path_obstacle : obstacles) {
    ...
    if (adc_left_l + ConfigParam::instance()->FLAGS_lateral_ignore_buffer <
            sl_boundary.start_l ||
        adc_right_l - ConfigParam::instance()->FLAGS_lateral_ignore_buffer >
            sl_boundary.end_l) {
      continue;
    }
