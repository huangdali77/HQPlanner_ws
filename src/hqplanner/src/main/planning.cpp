...
void Planning::RunOnce() {
  ...
  const PathObstacle* path_obs_adc = best_ref_info->path_decision().Find("0");
  const auto& adc_traj = path_obs_adc->obstacle()->Trajectory();
  if (adc_traj.trajectory_point.size() > 2) {
  ...
  // next_cycle_state.heading =
  // next_cycle_trajectory_point.path_point.theta; next_cycle_state.kappa =
  // next_cycle_trajectory_point.path_point.kappa;
  // next_cycle_state.linear_velocity = next_cycle_trajectory_point.v;
  // next_cycle_state.linear_acceleration = next_cycle_trajectory_point.a;
